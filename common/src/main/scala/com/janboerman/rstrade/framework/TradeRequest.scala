...
object TradeRequest {
    ...
    def removeAllRequests(player: Player): Unit = {
        incomingTradeRequests.remove(player)
        outgoingTradeRequests.remove(player)
    }
    ...
    def apply(initiator: Requester, target: Accepter)(implicit plugin: Plugin): TradeRequest = {
        ...
        plugin.getServer.getPluginManager.registerEvent(classOf[PlayerQuitEvent], listener, EventPriority.NORMAL, (_, event) => {
...
import com.janboerman.rstrade.framework.TradeRequest.{Accepter, Requester}
...
class TradeRequest private(val initiator: Requester,
                          ...
                          (implicit plugin: Plugin) {

    private val scheduler = plugin.getServer.getScheduler

    private var runningTimeoutTask: BukkitTask = newTimeoutTask()

    def newTimeoutTask(): BukkitTask =
        scheduler.runTaskLater(plugin, new Runnable {
            override def run(): Unit = unregister()
        }, 20L * 60)
