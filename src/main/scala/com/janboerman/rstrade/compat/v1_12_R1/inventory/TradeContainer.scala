...
/*
 * TODO [FUTURE UPDATE] actually I want the view to never delete items from the player's inventory (bottom inventory)
 * TODO [FUTURE UPDATE] so that no issues arise when the server shuts down unexpectedly.
 */

class TradeContainer(id: Int, val viewingPlayer: EntityHuman, val tradeInventory: TradeInventory) extends Container
    with Iterable[Slot]
    with IndexedSeq[Slot] {
...
//    private val buttons = tradeInventory.buttons
    ...
    override def iterator() : Iterator[Slot] = asScala(slots.iterator())
